...
// seed PRNG with a string; revert if not given (see read-me)
export function seed(text=false){
  rng=text?new alea(text):Math.random
}
...
export function bind(low,value,high=Number.MAX_VALUE){
...
export function choose(array,chancep=2){
  ...
  for(let i=0;i<=length-2;i+=1) if(chance(chancep))
    return array[i]
  return array[length-1]
}

// sums a positive and a negative roll
export function randomize(sides=6){
  return roll(1,sides)-roll(1,sides)
}
