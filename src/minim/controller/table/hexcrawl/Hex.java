...
public class Hex extends Table{
  ...
  public static final Table NEXT=new SimpleMetaTable("Hex (next)",List
      .of(Elevation.NEXT,Vegetation.NEXT,Water.NEXT,PointOfInterest.SINGLETON));
  static final List<String> CHARACTERISTICS=List.of("Elevation","Vegetation",
  ...
  public static class Hexagon{
    ...
    protected Hexagon(String elevationp,String vegetationp,String waterp){
      ...
      water=waterp;
      if(interest()) feature=PointOfInterest.TYPES.roll();
    }

    public Hexagon(){
      this(Elevation.SINGLETON.roll(),Vegetation.SINGLETON.roll(),
          Water.SINGLETON.roll());
    }
    ...
    public String toString(boolean full){
      ...
      if(water!=Water.DRY) s+=" by " + water.toLowerCase();
      if(full&&interest())
    ...
    static String next(String current,Table t,Table next,String lower,
        String higher){
      var nexte=next.roll();
      var l=t.lines;
      var i=l.indexOf(current);
      if(nexte==higher&&i>0) i-=1;
      else if(nexte==lower&&i<l.size()-1) i+=1;
      return l.get(i);
    }

    public Hexagon next(){
      var e=next(elevation,Elevation.SINGLETON,Elevation.NEXT,Elevation.LOWER,
          Elevation.HIGHER);
      var v=next(vegetation,Vegetation.SINGLETON,Vegetation.NEXT,
          Vegetation.SPARSER,Vegetation.DENSER);
